...
class AssertionAggregate implements AssertionInterface
{
     ...
     *
     * @param string $mode
     *            indicates how assertion chain result should interpreted
     *            (either 'all' or 'at_least_one')
     * @throws Exception
     *
     * @return \Zend\Permissions\Acl\Assertion\AssertionAggregate
     */
    public function setMode ($mode)
    {
        if ($mode != self::MODE_ALL && $mode != self::MODE_AT_LEAST_ONE) {
            throw new InvalidArgumentException(
                    'invalid assertion aggregate mode');
        }
            ...
            ResourceInterface $resource = null, $privilege = null)
    {
        ...
        if (! $this->assertions) {
            throw new RuntimeException(
                    'no assertion have been aggregated to this AssertionAggregate');
        }
            ...
            if (! $assertion instanceof AssertionInterface) {
                if (class_exists($assertion)) {
                    ...
                    if ($manager = $this->getAssertionManager()) {
                        ...
                        throw new RuntimeException(
                                'no assertion manager is set - cannot look up for assertions');
                    }
                }
            }
            
            $result = (bool) $assertion->assert($acl, $role, $resource, 
                    $privilege);
